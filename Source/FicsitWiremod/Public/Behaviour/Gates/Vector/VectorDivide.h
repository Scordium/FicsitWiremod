...
UCLASS()
class FICSITWIREMOD_API AVectorDivide : public AMultistateWiremodBuildable
{
	GENERATED_BODY()
...
public:
		...
		case 4:
			{
				...
				for (int i = 0; i < FMath::Max(ArrayA.Num(), ArrayB.Num()); i++)
				{
					...
					{
						const auto BDivider = ArrayB.IsValidIndex(i) ? ArrayB[i] : FVector::OneVector;
						
						ArrayOut.Add(ArrayA[i] / BDivider);
					}
	...
	virtual void GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const override
	{
		...
		DOREPLIFETIME(AVectorDivide, ArrayOut)
	}
